...
class BotCommands:
    ...
    async def handle_messages(self):
        ...
        if add_triggered:
            ...
            await self._handle_deletion_feed(self.server)
        elif help_triggered:
    ...
    async def _handle_deletion_feed(self, server_id):
        feed_name,_ = self._get_message_data()
    ...
    def _get_message_data(self):
        ...
        for i, elem in enumerate(user_cmd):
            if Utils.is_include_in_string("add", elem) \
                ...
                main_input_data = user_cmd[i+1]
            elif "channel" in elem:
                channel_submited = user_cmd[i+1]
        return main_input_data, channel_submited